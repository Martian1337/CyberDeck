...
sudo apt install massdns libldns-dev netexec -y
...
install_ad_tools() {
    ...
    install_trevorspray
    install_aux
    sudo apt install evil-winrm proxychains4 netexec pspt chisel -y
...
# Install Bloodhound
install_bloodhound() {
    # Install Neo4J placeholder (next 5 lines)
    #curl -fsSL https://debian.neo4j.com/neotechnology.gpg.key |sudo gpg --dearmor -o /usr/share/keyrings/neo4j.gpg
    #echo "deb [signed-by=/usr/share/keyrings/neo4j.gpg] https://debian.neo4j.com stable 4.1" | sudo tee -a /etc/apt/sources.list.d/neo4j.list
    #sudo apt install neo4j
    #sudo systemctl enable neo4j.servic
